...
export function 자판기연출({
  ...
  // ★ '밸브가 막 돌았다' 는 판정은 **모듈**이 기억한다(자판기밀기.연출봤나).
  //   여기 ref 로 두면 기차를 타고 내려 씬이 다시 마운트될 때 false 로 돌아가,
  //   이미 열린 자판기인데도 컷신이 한 번 더 돈다.
  //   씬이 새로 뜬 그 순간에는 "이미 열려 있다"는 것만 맞춰 두면 된다.
  ...
  useFrame((_, dt) => {
    ...
    // 열렸으면 '봤다'로 못 박는다 — 씬이 다시 떠도 두 번 돌지 않는다.
    //   되돌리기(열=0)면 다시 풀어 준다. 미리보기로 열어 둔 것도 컷신을 이미
    //   건너뛴 셈이라 같이 못 박는다(안 그러면 미리보기를 끄는 순간 컷신이 돈다).
    연출봤다치기(!!열);
