...
export function 소화전내부({
  ...
  // 밸브 ↔ 호스 이음 — 호스 뭉치 왼쪽 끝에서 내려와 밸브 커플링으로 들어간다.
  //   중심선을 그려 띠로 훑는다(호스와 같은 방식이라 이어진 한 줄로 보인다).
  const 이음호스 = useMemo(() => {
    const 시작 = [d * 반깊 * 0.1, 걸이y - (안높 - 칸경계) * 0.42, -안폭 * 0.3];
    const 끝 = [d * 반깊 * 0.28, -안높 * 0.3 + 0.26, -안폭 * 0.4];
    const 선 = [];
    const 마디 = 12;
    for (let i = 0; i <= 마디; i++) {
      const s = i / 마디;
      // 아래로 처졌다가 밸브로 빨려 들어간다
      const 처짐 = Math.sin(s * Math.PI) * 0.1;
      선.push([
        시작[0] + (끝[0] - 시작[0]) * s,
        시작[1] + (끝[1] - 시작[1]) * s - 처짐,
        시작[2] + (끝[2] - 시작[2]) * s,
  ...
  return (
    <group>
      ...
      {/* 개폐 밸브 — 함 **아래쪽 왼편**. 출구가 **위**를 본다.
             ...
             [자리]
               옆벽(안폭/2)까지 여유를 두고, 호스가 짧은 왼쪽 끝 아래를 쓴다. */}
      <group position={[d * 반깊 * 0.28, -안높 * 0.3, -안폭 * 0.4]}>
        {/* 뒤(급수관)에서 나오는 가로 관 */}
      ...
      {/* 밸브 ↔ 호스 이음 — 호스 뭉치 왼쪽 끝에서 내려와 밸브 커플링에 물린다.
             이게 없으면 밸브와 호스가 남남으로 놓인 두 물건으로 보인다. */}
      <mesh geometry={이음호스} castShadow>
        {툰(호스색, 밝기 * 0.95)}
        <만화선 선={선} />
      </mesh>
      ...
      <group position={[d * 반깊 * 0.24, 걸이y - 0.35, 안폭 * 0.4]}>
        ...
        <mesh position={[0, -0.01, 0]} castShadow>
          <cylinderGeometry args={[0.058, 0.058, 0.07, 12]} />
          {툰("#2a2c30", 밝기)}
        </mesh>
        {/* 호스와 물리는 황동 커플링 */}
