...
function 커피배출부({
  ...
  useFrame((_, dt) => {
    ...
    // Leva 로 강제로 연 값이 더 크면 그쪽을 따른다(연출 확인용)
    o.rotation.x = -Math.max(열림.current, 문열림) * 0.55;
  });
  ...
  return (
    <group position={[0, y, zf]}>
      ...
      {(() => {
        ...
        return (
          <group position={[0, 경첩y, 0.06]} ref={문ref}>
            {자판기id && (
              <상호대상
                id={`자판기문:${자판기id}`}
                반경={0.6}
...
function 상품버튼({
  ...
  useFrame(() => {
    ...
    // ② 불빛 — 돈을 넣으면 켜지고, 고른 버튼만 깜빡이다 꺼진다.
    //    겨냥 중에는 아주 살짝 밝혀 '누를 수 있는 것'임을 알린다(글자 없이).
    const k = 1 + 불 * 1.25 + 겨냥중 * 0.22;
    const 재질 = m.material;
    if (Array.isArray(재질) && 재질.length >= 4) {
      재질[0].color.copy(바탕.얼굴).multiplyScalar(라벨텍 ? 얼굴밝기 * k : k);
      재질[1].color.copy(바탕.위).multiplyScalar(k);
      재질[2].color.copy(바탕.아래).multiplyScalar(k);
      재질[3].color.copy(바탕.옆).multiplyScalar(k);
    }
  });
