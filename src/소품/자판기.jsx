...
export function 캔자판기({
  ...
  const 천 = 높이 - 테 - 0.78;
  const 바닥 = 테 + 0.15;
  const 배출높 = 0.85; // 배출구 구역 — 더 줄임
  const 제어높 = 0.55; // 버튼 줄(음료 선택 버튼)
  const 광고높 = 0.9; // 광고판 — 더 줄임
  const 광고하 = 바닥 + 배출높;
  const 광고상 = 광고하 + 광고높;
  const 제어상 = 광고상 + 제어높;
  const 창하 = 제어상; // 유리창(캔) 아래 = 버튼줄 위
  const 창상 = 천;
  const 창높 = 창상 - 창하; // 창이 커져 3줄이 안 겹친다
  const 창폭 = 폭 - 테 * 2;
  const 밴드 = 창높 / 단수; // 캔 한 줄이 차지하는 세로 폭

  const 캔높 = 0.62; // 슬림 캔 비율
  const 캔반지름 = Math.min(0.19, (창폭 / 열수) * 0.36);
  ...
  const 선반 = useMemo(() => {
    ...
    for (let s = 0; s < 단수; s++) {
      const y = 창하 + 밴드 * s + 0.1;
      것.push({ 크기: [창폭 - 0.1, 0.08, 1.0], 위치: [0, y, -0.15] });
    }
    return 상자합치기(것);
  }, [단수, 창하, 밴드, 창폭]);
