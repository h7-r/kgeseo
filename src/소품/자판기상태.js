...
const 기본 = () => ({
  ...
  캔때: 0, // 캔이 배출된 시각(ms) — 떨어져 튕겨 눕는 연출의 시작점
  컵: false, // 종이컵이 나왔나 (커피 자판기)
  컵때: 0, // 종이컵이 배출된 시각(ms) — 컵이 내려오는 연출의 시작점
  선택: null, // "핫" | "아이스"
  힌트종이: false, // 파란 캔과 함께 나온 '밸브 힌트 종이'가 아직 배출구에 있나
});
...
export function 버튼누르기(id, i, { 종류 = "음료", 온도 = null } = {}) {
  ...
  s.유효 = s.돈;
  소리재생("버튼", { 볼륨: 0.9 }); // 버튼 누르는 딸깍 소리(앞부분만 자른 파일)
  if (s.돈) {
    s.돈 = false; // 한 번 넣은 돈으로 하나
    if (종류 === "음료") {
      s.나온것 = i;
      s.캔때 = performance.now(); // 지금부터 캔이 떨어져 튕겨 눕는다
      // 캔이 수거함으로 '쿵' — 무겁게 가공한 소리. 배출 연출에 맞춰 살짝 늦게.
      setTimeout(() => 소리재생("캔배출", { 볼륨: 0.9 }), 90);
    } else {
      s.컵 = true;
      s.컵때 = performance.now(); // 지금부터 종이컵이 내려온다
      s.선택 = 온도;
      소리재생("컵배출", { 볼륨: 0.9 }); // 종이컵 떨어져 나오는 소리
    }
  }
  바뀜();
}
...
/** 밸브 힌트 종이의 배출구 대기 여부를 설정한다(파란 캔=참, 집어 가면 거짓). */
export function 힌트대기(id, v) {
  const s = 꺼내기(id);
  if (s.힌트종이 === v) return;
  s.힌트종이 = v;
  바뀜();
...
//
// ★ 돈이 없어도 **누른 버튼은 빛난다.**
//   버튼을 눌렀는데 아무 빛도 안 나면 "안 눌렸나?" 싶다. 실물 자판기도
//   눌린 버튼에는 불이 들어온다. 돈이 없으면 빛만 나고 아무것도 안 나온다.
//   (돈을 쓴 경우엔 깜빡임이 끝난 뒤 돈이 없으므로 자연히 꺼진다)
const 깜빡시간 = 1.1;
const 깜빡주기 = 0.16;
