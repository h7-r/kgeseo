...
// ═══════════════════════════════════════════════════════════════
// [무엇이 문제였나]
//   three 는 기본으로 **매 프레임 그림자 맵을 통째로 다시 그린다.** 그림자를
//   만드는 빛이 하나뿐이고 그 빛은 절대 안 움직이는데도 그렇다.
//   실제로 재 보니 방 한가운데에서 한 프레임 10.43ms 중 **5.78ms(55%)** 가
//   그림자 다시 그리기였다(그림자 갱신만 꺼 보니 4.65ms).
//
// [어떻게 고치나]
//   자동 갱신을 끄고, **필요할 때만** 다시 그린다.
//     ① 뭔가 움직였다는 신호가 오면 그 뒤 잠깐 동안은 매 프레임 (문이 열리는
//        동안, 자물쇠가 떨어지는 동안처럼 **여러 프레임에 걸친 움직임**을 덮는다)
//     ② 그 밖에는 느리게(기본 20프레임에 한 번). 이게 있어야 신호를 깜빡 잊은
//        움직임이 있어도 **그림자가 영영 굳어 버리지 않는다.** 늦어도 1/3초면 따라온다.
//
// [왜 '신호'를 밖에서 주나]
//   움직이는 것은 문·자물쇠·들고 다니는 물건·서랍처럼 여기저기 흩어져 있다.
//   그쪽에서 "움직였다" 한마디만 주면 되게 해 두면, 새 물건이 생겨도
//   그 한 줄만 부르면 된다(안 불러도 ② 덕분에 조용히 따라온다).
let _그림자급함 = 0;
const 지금초 = () =>
  (typeof performance !== "undefined" ? performance.now() : Date.now()) / 1000;

/** 뭔가 움직였다 — 이 시간(초) 동안 그림자를 매 프레임 다시 그린다 */
function 그림자흔들기(초 = 1.5) {
  _그림자급함 = Math.max(_그림자급함, 지금초() + 초);
}

/**
 * Canvas 안에 한 번만 놓는다. 그림자 맵 갱신을 관리한다.
 * @param 느린주기 조용할 때 몇 프레임마다 한 번 다시 그릴지
 */
function 그림자관리({ 느린주기 = 240, 급할때간격 = 3, 켬 = true }) {
  const { gl } = useThree();
  const 셈 = useRef(0);
  const 마지막 = useRef(-999);
  useEffect(() => {
    if (!켬) return;
    gl.shadowMap.autoUpdate = false;
    gl.shadowMap.needsUpdate = true; // 처음 한 번은 반드시 그린다
    그림자흔들기(6); // GLB 가 늦게 붙으므로 처음 몇 초는 넉넉히
    return () => {
      gl.shadowMap.autoUpdate = true; // 껐다 켤 때 원래대로
    };
  }, [gl, 켬]);
  useFrame(() => {
    if (!켬) return;
    셈.current++;
    const 급함 = 지금초() < _그림자급함;
    // ★ **그림자 한 번 다시 그리는 값이 12ms 다**(캐스터 496개 · 2048² 맵을 재 봤다).
    //   화면 한 장 그리는 값(8ms)보다 크다. 그래서:
    //   ① 움직이는 동안에도 **매 프레임은 안 그린다.** 급할때간격 프레임마다
    //      한 번이면 그림자가 최대 그만큼 늦게 따라올 뿐인데(눈에 안 띈다),
    //      값은 1/간격로 준다. 전에는 물건을 들고 걷는 내내 20ms 였다.
    //   ② 조용할 때 도는 안전망 주기를 20 → 240 프레임으로 늘린다.
    //      20 이면 **초당 세 번 12ms 가 튀어** 그게 곧 '끊김'이었다.
    //      안전망은 '신호를 깜빡한 물건'을 늦게라도 따라잡는 장치라 4초면 된다.
    const 때 =
      급함
        ? 셈.current - 마지막.current >= Math.max(1, 급할때간격)
        : 셈.current % 느린주기 === 0;
    if (때) {
      마지막.current = 셈.current;
...
// ── 셰이더 미리 데우기 ──────────────────────────────────
// [왜 필요한가]
//   three 는 **그 재질을 처음 그리는 순간** 셰이더를 컴파일한다. 그 한 프레임이
//   수백 ms 다 — 재 보니 먼지 재질 하나가 붙을 때 325ms 였고(프로그램 72→73),
//   그게 곧 "중간중간 렉" 으로 느껴진다. 게임이 시작되기 전에 미리 끝내 둔다.
// [두 번 하는 이유]
//   GLB 는 늦게 붙는다. 처음 한 번으로는 모델 재질이 빠진다.
function 셰이더예열({ 켬 = true, 늦게 = 3, 되풀이 = 2, 늘림 = 1.6, 최대쉼 = 20 }) {
  const { gl, scene, camera } = useThree();
  useEffect(() => {
    if (!켬) return;
    let 살아있음 = true;
    let 앞프로그램수 = -1; // 지난번에 세어 둔 셰이더 프로그램 수
    let 쉼 = Math.max(1, 되풀이) * 1000; // 다음 예열까지 기다릴 시간
    let 다음 = null;
    const 데우기 = () => {
      if (!살아있음) return;
      try {
        // compileAsync 가 있으면 그걸로 — 브라우저가 병렬 컴파일을 지원하면
        //   메인 스레드를 덜 막는다(없으면 동기 compile).
        if (gl.compileAsync) gl.compileAsync(scene, camera);
        else gl.compile(scene, camera);
        // ★ 새로 컴파일된 것이 **있을 때만** 그림자를 흔든다.
        //   [전에 무엇이 문제였나]  예열할 때마다 무조건 흔들었다. 예열이
        //   2 초마다 돌았으니 그림자 맵도 2 초마다 되살아났고, 그러면
        //   그림자관리가 아껴 놓은 것을 예열이 도로 까먹는 꼴이었다.
        //   프로그램 수가 그대로면 **새로 컴파일된 것이 없다** = 흔들 이유도 없다.
        // ★ compile() 은 **그림자용(깊이) 셰이더를 데우지 않는다.** 그래서
        //   그림자맵에 처음 들어가는 물건이 생길 때 또 컴파일이 튄다(재 보니
        //   컷신 도중 10개가 그렇게 새로 컴파일됐다). 그림자도 한 번 돌려 둔다.
        const 지금수 = gl.info?.programs?.length ?? -1;
        if (지금수 !== 앞프로그램수) {
          앞프로그램수 = 지금수;
          쉼 = Math.max(1, 되풀이) * 1000; // 뭔가 들어왔다 → 다시 촘촘히 살핀다
          그림자흔들기(0.4);
        } else {
          // 조용하면 **점점 뜸하게** 본다. 씬이 다 자리 잡은 뒤로는
          //   compile() 한 번이 그냥 낭비다(씬 전체를 훑고 재질마다 캐시를 뒤진다).
          쉼 = Math.min(최대쉼 * 1000, 쉼 * 늘림);
        }
      } catch {
        /* 예열은 실패해도 게임은 그대로 돈다 — 조용히 넘긴다 */
      }
      // ★ 예약은 **언제나 하나만** 둔다. t1·t2 가 각각 줄을 만들면 예열이
      //   두 줄로 돌면서 간격이 절반이 된다(그래서 먼저 것을 지우고 건다).
      if (다음) clearTimeout(다음);
      다음 = setTimeout(데우기, 쉼);
    };
    // ★ 한두 번으로는 모자란다. 늦게 붙는 GLB, 구역 최적화로 뒤늦게 올라오는
    //   복도·방, 상태에 따라 생기는 부품(먼지·꽂힌 관창…)의 재질은 그 뒤에 온다.
    //   재 보니 예열을 두 번만 했을 때 컷신 도중 11개가 새로 컴파일됐다.
    //   그래서 **되풀이해서** 데우되, 조용해지면 간격을 늘려 뒀다가
    //   새 재질이 나타나면 다시 촘촘해진다(위 프로그램 수 비교).
    const t1 = setTimeout(데우기, 400);
    const t2 = setTimeout(데우기, Math.max(0.5, 늦게) * 1000);
    return () => {
      살아있음 = false;
      clearTimeout(t1);
      clearTimeout(t2);
  ...
  }, [gl, scene, camera, 켬, 늦게, 되풀이, 늘림, 최대쉼]);
  return null;
}
...
/**
 * 브라우저에 저장된 Leva 값을 통째로 지운다.
 *   ★ Leva 「★ 전체값 출력 > 저장값초기화」 가 쓴다. 전에는 그 버튼이 App.jsx 안에서
 *     localStorage.removeItem(LEVA_KEY) 를 부르고 있었는데, LEVA_KEY 는 여기 있는
 *     이름이라 App 에서는 **없는 값**이었다 — 누르면 그 자리에서 터졌다(빌드는 통과).
 *     키를 아는 쪽이 지우는 게 맞다.
 */
function 저장전부지우기() {
  try {
...
const 강제기본값 = {
  // ※ 여기 있던 「폐역 조명」·「비밀 복도」 두 줄을 지웠다 — **죽은 줄이었다.**
  //   같은 객체 아래쪽(스탠드(공통) 뒤)에 같은 이름의 키가 또 있어서,
  //   자바스크립트 객체 규칙대로 **나중 것이 이기고 이 두 줄은 무시**되고 있었다
  //   (eslint no-dupe-keys 가 잡아 준 것). 지워도 동작은 한 줄도 안 바뀐다.
  //   ★ 합치지 않는 이유: 합치면 여기 있던 「반구광밝기 · 앰버포인트밝기」가
  //     **지금 손으로 맞춰 둔 값을 코드 기본값으로 되돌린다.** 조명이 바뀐다.
  //     되돌리기는 이미 오래전에 한 번 끝난 일이라 되살릴 이유가 없다.
  // ※ 서류 더미 높이는 여기서 되돌리지 않는다.
  //   이제 App 이 **책상 윗면을 재서** 높이를 정하므로(「책상에맞추기」, 기본 켬),
  //   저장된 옛 높이는 애초에 쓰이지 않는다. 스위치를 끄고 손으로 맞춘 값은
  //   그대로 남아야 하니, 여기서 지워 버리면 안 된다.
  // 자물쇠 — 만들면서 크기를 여러 번 줄였는데, 처음 켰을 때 저장된 값이
  //   계속 이겨서 **화면은 하나도 안 바뀌고 있었다.** 한 번 코드 값으로 되돌린다.
  // 걸쇠 두 장 — 자물쇠와 폴더를 나눴다. 이름이 바뀌었으니 한 번 되돌린다.
  "걸쇠 문쪽": [
    "보이기", "쇠막대맞춤", "좌우", "위아래", "깊이", "회전x", "회전y", "회전z",
    "구멍", "판폭", "두께", "길이", "날개", "나사수", "나사날개", "나사크기",
    "나사색", "색",
  ],
  "걸쇠 테두리쪽": [
    "보이기", "쇠막대맞춤", "좌우", "위아래", "깊이", "회전x", "회전y", "회전z",
  ...
  "소화전 자물쇠": [
    "보이기", "숨길때길보기", "자물쇠좌우", "자물쇠위아래", "자물쇠깊이", "크기", "보이기", "숨길때길보기",
    "폭", "높이", "깊이", "고리반지름", "고리굵기", "고리높이",
    "칸수", "다이얼띠", "x", "y", "z", "회전도",
    "맞춤1", "맞춤2", "맞춤3", "맞춤4", "맞춤5",
    "쇠색", "다이얼색", "글자색", "칸선", "칸선색", "칸선굵기", "칸선높이",
    "옆둥글기", "정답", "조작거리",
  ],
...
function useSavedControls(폴더, 스키마) {
  ...
  // 값을 문자열로 바꿔 '진짜 바뀌었을 때만' 저장한다.
  // ★ 그 문자열 만들기를 useMemo 로 막아 둔다.
  //   leva 는 zustand 의 shallow 비교를 써서 **값이 그대로면 같은 객체**를 돌려준다.
  //   그래서 참조만 보면 '안 바뀌었다'를 알 수 있고, 굳이 매번 만들 필요가 없다.
  //   역 씬에는 이 훅이 84번 불린다 — 서랍 한 칸 여는 동안에도 리렌더가 네 번 오는데,
  //   그때마다 84번씩 문자열을 만들고 버리던 것을 0번으로 줄인다.
  const json = useMemo(() => JSON.stringify(값), [값]);
  useEffect(() => {
    const 전체 = 저장읽기();
...
//   R3F 는 delta 를 손대지 않고 clock.getDelta() 를 그대로 넘긴다 — 상한이 없다.
//   씬을 갈아탈 때 메인 스레드가 0.5초쯤 멈추면, 그 다음 프레임의 dt 가 0.5 가 되고
//   그 한 프레임에 걸음이 3~10유닛 나간다. 막힘 판정은 '도착점 한 점'만 보므로
//   그 거리를 뛰면 벽도 가구도 그대로 뚫고 지나간다(플레이어 반경은 0.6뿐이다).
//   기차에서 내리는 순간 엉뚱한 자리에 가 있던 것이 이것이다.
//   1/30 로 자르면 최악의 경우도 한 걸음(달려도 0.34유닛)이라 아무것도 못 뚫는다.
//   느린 기기에서는 그만큼 세상이 느리게 흐르는 셈인데, 순간이동보다 낫다.
const DT최대 = 1 / 30;
...
// 걷기 계산에 돌려 쓰는 그릇 — 매 프레임 새로 만들지 않으려고 바깥에 둔다
const _이동앞 = new THREE.Vector3();
const _이동옆 = new THREE.Vector3();
const _이동갈 = new THREE.Vector3();
...
function use이동(
  ...
  // ★ useFrame 의 '첫 프레임'이 아니라 **마운트 커밋 시점**에 한다.
  //   useFrame 은 렌더 다음 rAF 에서 도는지라, 그 사이에 새 씬이 **이전 씬 좌표로
  //   한 프레임 그려질** 수 있다. 기차에서 내리는 순간 엉뚱한 데가 한 번 번쩍이던 것이
  //   그것이다. useLayoutEffect 는 커밋 중에 돌아서 화면에 나가기 전에 끝난다.
  //   deps 가 빈 배열인 이유 = 이 자리잡기는 씬에 들어설 때 딱 한 번만 해야 한다.
  //   (그 뒤에 시작/바라봄 이 바뀌었다고 걷던 사람을 도로 끌어다 놓으면 안 된다)
  useLayoutEffect(() => {
    if (시작) {
      camera.position.set(시작[0], 시작[1] ?? eyeRef.current, 시작[2]);
  ...
  useFrame((_, 원본dt) => {
    ...
    const k = keys.current;
    // ★ 벡터를 매 프레임 새로 만들지 않는다.
    //   초당 60번 × 3개 = 180개가 1초마다 쓰레기가 된다. 걷는 동안 계속
    //   메모리를 갉아 두면, 청소(GC)가 도는 그 한 프레임이 눈에 띄게 끊긴다.
    //   이동은 늘 한 사람뿐이라 그릇 세 개를 모듈에 두고 돌려 쓰면 된다.
    const fwd = _이동앞, right = _이동옆, wish = _이동갈;
    ...
    // ── 뛰는 발소리(루프) — 달리기 키 + 바닥 + 실제 이동일 때만 ──
    const 뛰는중 =
      active && k.run && !crouching && grounded.current && target.lengthSq() > 1e-6;
