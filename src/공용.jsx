...
function 셰이더예열({ 켬 = true, 늦게 = 3, 되풀이 = 2 }) {
  ...
  useEffect(() => {
    ...
    //   그래서 **주기적으로** 데운다 — 이미 데워진 장면은 캐시만 확인하고 끝난다.
    const t1 = setTimeout(데우기, 400);
    const t2 = setTimeout(데우기, Math.max(0.5, 늦게) * 1000);
    const 주기 = setInterval(데우기, Math.max(1, 되풀이) * 1000);
    return () => {
      살아있음 = false;
      clearTimeout(t1);
      clearTimeout(t2);
...
const 강제기본값 = {
  // 밝기 한 단 올리기 — 수사관실(폐역 조명)·비밀 복도가 전체적으로 어둡다는 요청.
  //   코드 기본값만 올리면 이미 만져 본 슬라이더의 저장값이 이겨서 화면이 그대로다.
  //   딱 한 번 되돌린 뒤에는 다시 손으로 맞춘 값이 살아난다.
  "폐역 조명": ["기본광밝기", "반구광밝기", "주광밝기", "앰버포인트밝기"],
  "비밀 복도": ["최소밝기", "벽밝기"],
