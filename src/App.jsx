...
function Scene({ active, onNear, controlsRef, onLockChange }) {
  ...
  const 소화전속 = useSavedControls("소화전 속", {
    ...
    경종속크기: { value: 0.08, min: 0.02, max: 0.4, step: 0.005 },
    경종높이: { value: 0, min: -0.3, max: 0.3, step: 0.005 }, // 경종 전체
    경종속위아래: { value: 0, min: -0.15, max: 0.15, step: 0.005 }, // 속 원만
    // ── 발신기(누름 버튼) — 경종처럼 바깥/속을 따로 ──
    발신기바깥색: "#ffffff",
    발신기속색: "#c0392b",
    발신기크기: { value: 0.18, min: 0.06, max: 0.4, step: 0.005 },
    발신기속크기: { value: 0.5, min: 0.1, max: 0.9, step: 0.02 }, // 바깥 대비
    // ── 위치표시등 ──
    표시등색: "#ffffff",
    // 0 이면 안 빛난다. 화면 Bloom 임계값(0.85)을 넘겨야 외곽이 번진다.
    빛세기: { value: 1.5, min: 0, max: 3, step: 0.05 },
