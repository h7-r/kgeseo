...
function 벽함({
  ...
  const 홈테지오 = useMemo(() => {
    ...
    const yB = cy - 홈높 / 2 - 홈테,
      yT = cy + 홈높 / 2 + 홈테;
    return 상자합치기([
      { 크기: [두께, 홈높 + 홈테 * 2, 홈테], 위치: [x0, cy, zL + 홈테 / 2] },
      { 크기: [두께, 홈높 + 홈테 * 2, 홈테], 위치: [x0, cy, zR - 홈테 / 2] },
      { 크기: [두께, 홈테, 홈폭], 위치: [x0, yT - 홈테 / 2, 홈z] },
      { 크기: [두께, 홈테, 홈폭], 위치: [x0, yB + 홈테 / 2, 홈z] },
      // 손가락 턱 — 홈 안쪽 아래에 걸친 작은 단. 여기 손가락을 걸고 당긴다.
      {
        크기: [0.045, 0.028, 홈폭],
        위치: [d * (깊이 / 2 + 0.05), cy - 홈높 / 2 + 0.024, 홈z],
      },
    ]);
  }, [d, 깊이, cy, 홈폭, 홈높, 홈테, 홈z]);
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      <mesh position={[d * (깊이 / 2 + 0.015), cy, 홈z]}>
...
function Scene({ active, onNear, controlsRef, onLockChange }) {
  ...
  const CD = useSavedControls("비밀 복도", {
    ...
    잡동사니밀도: { value: 0.55, min: 0, max: 2, step: 0.05 },
    ...
    부식보이기: true,
    부식바닥수: { value: 16, min: 0, max: 60, step: 1 },
    부식벽수: { value: 26, min: 0, max: 80, step: 1 },
