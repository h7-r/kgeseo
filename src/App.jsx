...
import { 열렸나, 여닫기, use열렸나 } from "./소품/여닫이.js";
// ── 화면 위에 뜨는 창들 ─────────────────────────────────────
...
function 벽함({
  ...
  const 관 = useMemo(() => {
    ...
    if (관모양 === "위") {
      ...
      for (const sz of [-1, 1]) {
        ...
        g.translate(x, (위 + 아래) / 2, sz * 0.32);
        조각.push(g);
      }
    } else {
      // 꺾임 중심(로컬 z, 세계 y). 안쪽 관이 z = −간격/2 로 내려오게 맞춘다.
      const Cz = -관간격 / 2 - 관굽힘;
      const Cy = 관꺾임높이 + 관굽힘;
      const 끝 = 관끝z - z; // 세계 → 이 함 기준 로컬
      if (바닥높이 <= Cy) return null;
      for (let i = 0; i < 2; i++) {
        const R = 관굽힘 + i * 관간격;
        const 세로z = Cz + R; // 내려오는 자리
        const 가로y = Cy - R; // 꺾인 뒤 높이
        ...
        e.translate(x, Cy, Cz);
        조각.push(e);
        // ③ 꺾인 뒤 벽을 따라 옆으로. 끝이 어느 쪽이든 되게 부호를 따진다.
  ...
  }, [전선관, 관모양, 천장높이, 바닥높이, 높이, d, 깊이, 관끝z, z, 관꺾임높이, 관간격, 관굽힘]);
...
function Scene({ active, onNear, controlsRef, onLockChange }) {
  ...
  const CD = useSavedControls("비밀 복도", {
    ...
    // ── 소화전 배선관 — 함 아래로 내려와 꺾여서 옆 자판기 뒤까지 ──
    //   배전반은 위(천장)로 올라가고, 이쪽은 아래로 내려와 벽을 따라 간다.
    소화전관: true,
    //   끝나는 자리. 기본값은 음료 자판기 한가운데라 기계 뒤로 숨는다.
    소화전관끝z: { value: 9.2, min: -60, max: 26, step: 0.1 },
    소화전관높이: { value: 1.1, min: 0.3, max: 3, step: 0.05 }, // 꺾이는 높이
    소화전관간격: { value: 0.24, min: 0.08, max: 0.8, step: 0.01 },
    소화전관굽힘: { value: 0.3, min: 0.08, max: 0.9, step: 0.02 },
