...
function 벽함({
  ...
  useFrame((_, dt) => {
    ...
    o.rotation.y = d * 1.25 * 열림.current;
  });
...
function Scene({ active, onNear, controlsRef, onLockChange }) {
  ...
  const 소화전속 = useSavedControls("소화전 속", {
    ...
    금속색: "#9aa1a8",
    호스색: "#d9d3c2",
    // 위 칸 — 경종(종) · 발신기(누름 버튼) · 위치표시등
    경종바깥색: "#ffffff",
    경종속색: "#c0392b",
    발신기색: "#c0392b",
    표시등색: "#b4524a",
    경종크기: { value: 0.24, min: 0.08, max: 0.45, step: 0.005 },
    경종속크기: { value: 0.15, min: 0.03, max: 0.4, step: 0.005 },
    발신기크기: { value: 0.17, min: 0.06, max: 0.4, step: 0.005 },
    // 0 이면 안 빛난다. 화면 Bloom 임계값(0.85)을 넘겨야 외곽이 번진다.
    빛세기: { value: 0.9, min: 0, max: 3, step: 0.05 },
    // 뒤판에 얼마나 붙일지. 0 = 뒤판에 딱 붙음, 1 = 함 앞면까지 나옴.
    부품깊이: { value: 0.1, min: 0, max: 1, step: 0.02 },
