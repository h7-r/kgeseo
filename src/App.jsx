...
function PcSet({
  ...
  const 키보드조각 = useToon조각("/models/keyboard.glb", KB?.색 ?? "#2c2f34");
  const 마우스조각 = useToon조각("/models/mouse.glb", MS?.색 ?? "#2c2f34");
  ...
  return (
    ...
    <group position={[x, 0, z]} rotation={[0, rot, 0]}>
      ...
      {/* ── 키보드 ── 위치는 '모니터 기준 상대 좌표'.
          좌우 = 로컬 X, 앞뒤 = 로컬 Z(+가 사용자 쪽), 높이 = 모니터 y 에서의 차이 */}
      {KB?.보이기 !== false && (
        <group
          position={[
            KB?.좌우 ?? -0.31,
          ...
          rotation={[0, KB?.회전 ?? 0, 0]}
          // scale 에 숫자 하나 대신 배열 [x, y, z] 를 주면 축마다 다르게 늘린다.
          //   두께·깊이 기본값이 1 이라 평소엔 균일 배율 → 외곽선 두께도 사방이 고르다.
          //   (비율을 크게 어긋내면 눌린 축의 외곽선이 얇아지니 미세조정용으로만)
          scale={[
            (KB?.크기 ?? 1.55) * 1,
            (KB?.크기 ?? 1.55) * (KB?.두께 ?? 0.5),
            (KB?.크기 ?? 1.55) * (KB?.깊이 ?? 0.83),
          ]}
        >
          <조각그리기 조각={키보드조각} 선={KB선} />
        </group>
      )}

      {/* ── 마우스 ── 키보드 오른쪽. 휠·케이블이 +Z 라서 기본 회전을 180°로 돌려
          케이블이 모니터 쪽(-Z)을 향하게 했다. */}
      {MS?.보이기 !== false && (
        <group
          position={[
            MS?.좌우 ?? 0.82,
          ...
          rotation={[0, MS?.회전 ?? -2.98, 0]}
          scale={MS?.크기 ?? 0.37}
        >
          <조각그리기 조각={마우스조각} 선={MS선} />
        </group>
      )}
    </group>
  );
...
function 벽함({
  ...
  const 합본 = useMemo(() => {
    ...
    return 상자합치기([
      ...
      // 걸쇠(손잡이) — 반대쪽
      { 크기: [0.14, 0.34, 0.12], 위치: [d * (깊이 / 2 + 0.06), cy, 폭 / 2 - 0.12] },
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      {/* ② 문짝 — 함체보다 살짝 작고 앞으로 나와 '따로 열리는 판'으로 읽힌다 */}
      <mesh position={[d * (깊이 / 2 - 0.02), cy, 0]} castShadow>
      ...
      <mesh
        position={[d * (깊이 / 2 + 0.045), cy + 높이 * 0.12, 0]}
        rotation={[0, d > 0 ? Math.PI / 2 : -Math.PI / 2, 0]}
      >
        <planeGeometry args={[폭 * 0.74, 폭 * 0.74 * (208 / 320)]} />
...
function Scene({ active, onNear, controlsRef, onLockChange }) {
  ...
  const S = useSavedControls("스탠드(공통)", {
    천장등끄기: false,
    전체어둡게: { value: 0.73, min: 0, max: 1, step: 0.01 },
    켜기: true,
  ...
  const CD = useSavedControls("비밀 복도", {
    ...
    깊이어둠: { value: 0.66, min: 0, max: 1, step: 0.01 },
    감쇠거리: { value: 33, min: 5, max: 80, step: 1 },
    최소밝기: { value: 0.45, min: 0, max: 1, step: 0.01 }, // 감광 바닥값
    벽밝기: { value: 1.35, min: 0.5, max: 2.5, step: 0.05 }, // 벽 전체 배수
    // 비상계단(복도 끝) 쪽만 추가로 어둡게 — 복도가 더 길어 보이게 하는 장치
    끝쪽어둠: { value: 0.45, min: 0, max: 0.9, step: 0.01 },
  ...
  useEffect(() => {
    걸이등록("모자걸이", {
      ...
      rot: ht1.회전,
      // ★ 자빠진 각도까지 넘긴다.
      //   유령은 '놓으면 이렇게 된다'를 보여주는 그림이다. 각도를 안 넘기면
      //   유령은 반듯이 선 모자인데 E 를 누르면 비스듬히 걸려 딴 물건처럼 보인다.
      기울기: ht1.기울기,
      반경: 1.1,
    });
    return () => 걸이해제("모자걸이");
  }, [ht1.x, ht1.z, ht1.높이, ht1.회전, ht1.기울기]);
