...
// 이제 일반 주소에서도 캐릭터를 띄운다(기본=Meshy). 사이드킥 몸체만 주소로 켠다.
// ?avatar=chibi → 치비 몸체 시제품(1단계 검토). 이동·시점·펀치 연결은 Sidekick 테스트와 같다.
// ?avatar=meshy → Meshy 민머리 기본 모델(같은 런타임·패널, 몸체 파일만 다름)
// ★ 로비·비밀복도에는 이제 기본으로 캐릭터가 뜬다(황미르 연동본).
//   주소에 아무것도 안 붙이면 = Meshy 몸체 + 왼쪽 꾸미기 패널.
//   ?avatar=chibi → 치비 몸체 / ?avatar=sidekick → 사이드킥 몸체 로만 바꾼다.
const 로비치비몸체 = 쿼리.get("avatar") === "chibi" ? "chibi" : "meshy";
const 로비치비테스트 = 쿼리.get("avatar") !== "sidekick"; // 사이드킥만 아니면 치비/메시 경로
const 로비아바타테스트 = true;                              // 캐릭터는 항상 켠다
...
export default function App() {
  ...
  // ── 기차는 페이드로 넘어간다 ─────────────────────────────
  // [생각이 바뀐 이유]
  //   처음엔 페이드를 뺐다 — '문을 열고 걸어 들어가는' 흐름이 끊긴다고 봤다.
  //   그런데 역 씬은 지오메트리가 1100개가 넘는다. 갈아탈 때마다 그걸 통째로
  //   버리고 다시 만드는 동안 **메인 스레드가 수백 ms 멈춘다.** 가릴 게 없는 게
  //   아니라, 가려야 할 게 제일 큰 자리였다.
  //   맨눈으로 보면 화면이 얼어붙었다 툭 튀는데, 검은 막을 한 번 끼우면
  //   그 시간이 '문이 닫혔다 열리는 사이'로 읽힌다.
  //   ★ 막이 걷히는 시점(이동 후 120ms)은 **리액트가 일을 마쳐야** 온다.
  //     그래서 무거운 씬일수록 막이 저절로 더 오래 머문다 — 딱 필요한 만큼 가린다.
  //   ※ 이걸로 끊김이 없어지는 건 아니다. **가리는 것**이다.
  //     실제로 줄이려면 역 씬을 언마운트하지 않고 visible 로만 끄는 쪽으로 가야 한다.
