...
import { 소화전내부, 관창모양 } from "./소품/소화전내부.jsx";
import { 관창곳, use관창 } from "./소품/관창.js";
...
function 벽함({
  ...
  const 관 = useMemo(() => {
    ...
    관끝z, 관꺾임높이, 관간격, 관높이차, 관굽힘, 관굵기, 관앞으로, 관치우침,
  ]);
...
function Scene({ active, onNear }) {
  ...
  const 관창CD = useSavedControls("관창(손)", {
    앞: { value: 1.5, min: 0.4, max: 3, step: 0.05 },
    아래: { value: 0.6, min: -0.5, max: 2, step: 0.05 },
    옆: { value: 0.55, min: -1.5, max: 1.5, step: 0.05 },
    // 90 = 물 나오는 끝이 정면. 줄이면 끝이 아래로 내려간다.
    기울기: { value: 74, min: 0, max: 180, step: 2 },
    비틀기: { value: -14, min: -90, max: 90, step: 2 },
