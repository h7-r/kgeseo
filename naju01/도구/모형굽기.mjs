...
// ★ 빈 문자열을 **0 으로 읽지 않게** 막는다.
//   `Number("") === 0` 이다. `??` 는 빈 문자열을 통과시키므로,
//   크기를 건너뛰려고 `"" 9000` 처럼 넘기면 크기가 **0** 이 되고
//   `중심` 규약의 배율이 `0 / 가장긴쪽` → 0 이 되어 모형이 **한 점으로
//   찌그러진다.** 씬에는 올라오는데 화면에서 사라진다 — 실제로 바위 일곱 개가
//   그렇게 됐고, 지오메트리 경계상자가 0×0×0 인 걸 보고서야 잡았다.
const 크기 = 크기옵션 === undefined || 크기옵션 === "" ? 0.8 : Number(크기옵션);
if (!Number.isFinite(크기) || 크기 <= 0) {
  console.error(`크기가 이상하다: ${JSON.stringify(크기옵션)} → ${크기}`);
  process.exit(1);
}
