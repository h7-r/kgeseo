...
// ★ 코어 한복판에서 **0 이 되게** 기준값을 뺀다.
//   예전에는 안 뺐더니 코어 앞에서 이 함수가 늘 +17~18 m 를 더했다.
//   그래서 `강.건너` 를 46 으로 줄여 놨는데 실제 강폭은 **67 m** 였다
//   (실측: 포구 앞 66.5 m). 도면 값이 뜻하는 바와 화면이 어긋난 것이다.
//   기준을 빼면 `강.건너` 가 곧 「이쪽 물가에서 저편 물가까지」가 된다.
const 건너기준 = 18 * Math.sin(40 * 0.014 + 1.3); // 코어 한복판(x=40) 값
export const 건너굽이 = (x) =>
  18 * Math.sin(x * 0.014 + 1.3) - 건너기준 + 강굽이(x);
...
// ★ 지금은 **아무도 안 쓴다.** 원경(실제 지형 + 나무 + 택촌)이 들어오기 전에
//   강 건너를 대신하던 납작한 실루엣이다. 진짜 땅이 생긴 뒤로는 그 땅 앞에
//   종잇장이 하나 더 서는 꼴이라 씬에서 걷어냈다 — 「택촌 앞에 흰 가림막이
//   있어 마을이 안 보인다」로 돌아온 바로 그것이다(높이 17.3 m · 겹 2장).
//   되살릴 일이 생기면 **택촌보다 훨씬 뒤로** 보내고 높이를 낮춰야 한다.
