...
// ── 수목대 **자리만** 뽑는다 ────────────────────────────────
// [왜 생겼나]  `수목대만들기` 는 나무를 코드로 깎아 **통째로 하나의 메시로
//   합친다**(드로우콜 1 개). 값은 싸지만 대가가 둘이다 —
//     ① 편집기가 못 잡는다. 합쳐진 덩어리라 고를 인스턴스가 없다.
//     ② 품질이 한 그루 ≈ 80 삼각형이다(정이십면체 잎덩이 두셋).
//   실제로 Z4 에서 「이 나무들은 에셋이 아닌지 선택도 안 되고 퀄리티가
//   엉망」이라는 지적이 나왔다. 자리만 내주고 **심는 것은 인스턴스 무리**에
//   맡기면 둘 다 풀린다 — 구운 나무 모형이 들어가고 편집기가 고른다.
// ※ 값은 `수풀뿌리기` 의 나무자리와 **같은 형태** { x, z, y, 키 } 다.
export function 수목대자리들({
  X, Z, 바닥, 높이, 개수, 시드, 안쪽 = 0.5, 지면요철,
}) {
  const 딛는곳 = typeof 바닥 === "function" ? 바닥 : () => 바닥;
  const 뽑기 = makeRandom(시드 + 1);
  const x0 = X[0] + 안쪽, x1 = X[1] - 안쪽;
  const z0 = Z[0] + 안쪽, z1 = Z[1] - 안쪽;
  const 자리들 = [];
  for (let i = 0; i < 개수; i++) {
    const x = x0 + 뽑기() * (x1 - x0);
    const z = z0 + 뽑기() * (z1 - z0);
