...
export function 들판만들기({
  ...
  const 높이 = (x, z) => {
    ...
    //   ※ 예전에는 두 램프를 **`Math.max`** 로 묶었다. 그런데 앞 램프는
    //     z ≥ 이쪽 에서 1 로 잠기므로, max 를 쓰면 **강 건너 땅 전체가 영영 1**
    //     이 된다 — 즉 저편이 통째로 1.6 m 파인 채였다. 물이 z 139 까지 덮던
    //     때는 안 보였는데, 강을 30 m 로 좁히자 그 웅덩이가 **물 밖으로
    //     드러나** 택촌 앞에 창백한 띠가 됐다(실측: z 78~98 이 고도 −1.5 m).
    //   ※ `min` 이라야 「양쪽 물가 사이에서만 파인다」가 된다. 그리고 저편은
    //     **물 끝에서 정확히 0** 이 되게 잡는다 — 물이 안 덮는 자리는 파면 안 된다.
    const 이쪽 = 강.Z시작 + 강굽이(x);
    const 물끝 = 강.Z시작 + 강.건너 + 3 + 건너굽이(x); // 수면 메시의 저편 끝
    const 기슭 = Math.min(
      THREE.MathUtils.clamp((z - (이쪽 - 12)) / 12, 0, 1),
      THREE.MathUtils.clamp((물끝 - z) / 8, 0, 1),
    );
    y -= 1.6 * 기슭 * 기슭;
