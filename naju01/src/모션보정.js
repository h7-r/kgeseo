...
export const 기본보정 = {
  ...
  //   팔꿈치 경첩의 방향은 위팔의 롤이 정한다. 우리 클립은 팔꿈치가 옆으로 벌어지며 접혀
  //   (옆/뒤 성분비 1.14, 원본 0.21) 팔이 엇나가 보였다.
  //   ※ 다른 보정과 달리 **뒤에 곱한다**(뼈 제 축 둘레 회전). 앞에 곱하면 팔 전체가 움직인다.
  //   ※ 위팔은 소매에 덮인 원통이라 비틀려도 티가 안 난다. 아래팔에는 걸지 말 것.
  걷기팔롤도: 0,
  ...
  // 겹치는 깊이를 줄이는 데 쓴다.
  팔꿈치펴기늘: false,
  // 팔마다 다르게 펼 때(없으면 팔꿈치펴기도). 팔짱처럼 좌우가 다른 자세에 쓴다.
  왼팔꿈치펴기도: null,
  오른팔꿈치펴기도: null,
  ...
  몸흔듦배율: 0.55,
  // 걷기·달리기에서 척추의 좌우 비틀림(위축 둘레) 폭 배율. 1 = 클립 그대로, >1 이면 더 비튼다.
  // 팔을 흔들면 상체가 따라 도는 그 움직임이다. 골반은 빼고 척추만 — 골반을 비틀면 다리가 끌려간다.
  몸비틀배율: 1,
  ...
  목세움도: 0,
  // 머리를 숙인다(+ 아래). Tripo 대기 클립은 턱이 들려 하늘을 보고 있어 대기에서만 내린다.
  머리숙임도: 0,
  ...
  무릎벌림도: 3,
  // 서 있을 때 두 다리를 좌우로 벌린다(+ 바깥). 발은 바닥에 평평히 남게 반대로 돌린다.
  // 무릎벌림도와 달리 종아리를 되돌리지 않아 **발까지 함께 벌어진다** — 벌린 자세를 만드는 쪽이다.
  다리벌림도: 0,
...
export const 트리포보정 = {
  ...
  팔벌림도: 10, 걷기팔붙임도: 0, 걷기팔뒤로도: 0, 팔흔듦배율: 1, 팔꿈치펴기도: 0, 몸흔듦배율: 1, 몸비틀배율: 1,
  // ※ 좌우 팔 흔들림 폭은 클립이 3cm 어긋나 있다(원본 캐릭터 0.9cm). 축 성분으로 재서 평균에
  //   맞춰 봤더니 15cm 로 더 벌어졌다 — 팔 흔들림은 한 축 회전이 아니라 못 잰다. 그대로 둔다.
  // ※ 걷기팔롤도 는 0 이다. 40° 로 돌려 팔꿈치 방향을 잡아 봤더니 비틀림 뼈에 웨이트가 없어
  //   어깨 세모근이 통째로 돌아 어깨가 변형되고, 좌우 팔이 다른 각도로 흔들려 보였다(되돌림).
  걷기팔롤도: 0,
  골반기울기도: 0, 허리곡선도: 0, 허리세움도: 0, 목세움도: 0, 보폭배율: 1, 발피치도: 0, 무릎펴기도: 0,
  무릎벌림도: 0, 다리벌림도: 0, 머리숙임도: 0, 걷기숙임도: 0, 걷기다리앞도: 0, 걷기다리모음도: 0,
...
export const 트리포성별보정 = {
  ...
  // 걷기: 상체에 견줘 다리가 너무 모여 보였다. 조금만 벌린다(걷기다리모음도 음수 = 벌림).
  // 대기: 남녀가 같은 클립(허리에 손)을 쓰므로, 남성만 다리를 벌려 자세를 구분한다.
  masculine: {
    걷기숙임도: 14, 몸비틀배율: 1.4, 걷기가슴세움도: 15, 걷기목세움도: -5,
    왼발안쪽돌림도: 14, 오른발안쪽돌림도: 8, 걷기다리모음도: -3,
    // 골반 18° 는 상의 밑단은 잡아 줬지만 골반↔허벅지가 반대로 돌며 엉덩이 살이 꺾여
    // 걸을 때 허벅지 윗부분이 끊어져 보였다(여성은 이 보정이 없어 멀쩡했다). 절반으로 줄인다.
    걷기골반기울기도: 9,
    대기덧값: { 다리벌림도: 5, 머리숙임도: 10 },
  },
  ...
  feminine: { 대기덧값: { 허리곡선도: -20, 골반기울기도: -20, 머리숙임도: 10 } },
};
...
// 제 길이 축 둘레로 돌릴 뼈 — [뼈, 자식(길이 방향을 재는 뼈), 부호]
const 팔롤본 = [["upperarm_l", "lowerarm_l", 1], ["upperarm_r", "lowerarm_r", -1]];
...
const 자세보정 = (값) => [
  ...
  ["head", 옆축, 값.머리숙임도, false],
  ["foot_l", 옆축, -값.발피치도, false],
  ["foot_r", 옆축, -값.발피치도, false],
  // 무릎 벌리기 — 앞축 둘레. 왼다리는 +가 바깥.
  ["thigh_l", 앞축, 값.다리벌림도, false],
  ["thigh_r", 앞축, -값.다리벌림도, false],
  ["foot_l", 앞축, -값.다리벌림도, false],
  ["foot_r", 앞축, 값.다리벌림도, false],
...
export function 보정쿼터니언(skin, 값) {
  ...
  팔롤본.forEach(([name, childName, 부호]) => {
    const bone = skin.skeleton.getBoneByName(name);
    const child = skin.skeleton.getBoneByName(childName);
    if (!bone || !child) return;
    // 자식 위치는 이 뼈의 부모 기준이라, 이 뼈 자신의 기준으로 옮겨야 제 길이 축이 된다.
    const 축 = child.position.clone().normalize().applyQuaternion(bone.quaternion.clone().invert()).normalize();
    out.set(name, { ...(out.get(name) ?? {}), 롤축: 축, 롤부호: 부호 });
  });
  몸비틀본.forEach((name) => {
    const bone = skin.skeleton.getBoneByName(name);
    if (!bone) return;
    const 세계 = 부모쉴때회전(bone).multiply(bone.quaternion);
    const 축 = 위축.clone().applyQuaternion(세계.invert()).normalize();
    out.set(name, { ...(out.get(name) ?? {}), 비틀축: 축 });
  });
  return out;
}
...
export function 클립보정(clip, 보정, 이름, 값) {
  ...
  순서.forEach((track) => {
    ...
    if (규칙.팔꿈치 && (이동중 || 값.팔꿈치펴기늘)) {
      const 도 = (이름[1].endsWith("_l") ? 값.왼팔꿈치펴기도 : 값.오른팔꿈치펴기도) ?? 값.팔꿈치펴기도;
      각도빼기(track, 규칙.팔꿈치, 도);
    }
    ...
    if (규칙.롤축 && 이동중 && 값.걷기팔롤도) {
      const 롤 = new THREE.Quaternion().setFromAxisAngle(규칙.롤축, THREE.MathUtils.degToRad(값.걷기팔롤도 * 규칙.롤부호));
      for (let i = 0; i < track.values.length; i += 4) {
        q.fromArray(track.values, i).multiply(롤);
