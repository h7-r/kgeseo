...
export function 계기판({ 보고 }) {
  ...
  return (
    <div style={S.판}>
      ...
      {/* 지금 밟고 있는 땅이 어느 쪽인가 — 옛 코드 지형인가, 블렌더가 구운
          한 벌인가. 둘이 겉보기로 비슷할 수 있어서 **글자로** 박아 둔다.
          읽기에 실패하면 조용히 옛 지형으로 돌아가므로 그것도 적는다. */}
      {v.지형출처 ? (
        <div style={S.줄}>
          <span style={S.흐림}>지형</span>
          <span style={v.지형출처.startsWith("새(") ? { color: "#8FD6A0" } : S.흐림}>
            {v.지형출처}
          </span>
        </div>
      ) : null}
