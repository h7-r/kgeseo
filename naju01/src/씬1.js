...
export function 구렁이표본들() {
  ...
  g.setIndex(new THREE.BufferAttribute(풀기(구렁이모형.인덱스, Uint16Array), 1));
  g.setAttribute(
    "position",
    new THREE.BufferAttribute(풀기(구렁이모형.위치, Float32Array), 3),
  );
  // 노멀은 구운 파일에 없다 — 여기서 만든다.
  //   ※ `toNonIndexed()` 를 먼저 해야 **면마다 각진**(flat) 음영이 나온다.
  //     인덱스를 살린 채로 계산하면 꼭짓점을 공유해 매끈해지는데, 그러면
  //     이 그레이박스의 저폴리 화풍에서 혼자만 미끈거려 겉돈다.
  const 면 = g.toNonIndexed();
  g.dispose();
  면.computeVertexNormals();
  // 꼭짓점 색 — 등은 어둡고 배는 밝게, **비율**로만 굽는다.
  //   [왜 반드시 색 속성이 있어야 하나]  재질이 `vertexColors: true` 인데
  //   `color` 속성이 없으면 three 가 없는 값을 **(0,0,0)** 으로 읽어 통째로
  //   **검게** 나온다. 이 프로젝트에서 세 번 겪은 함정이다
  //   (바위 · 팔레트 썸네일 · 표본 전반).
  //   [왜 절대값이 아니라 비율인가]  실제 색은 `instanceColor` 가 **곱한다.**
  //   여기에 진짜 색을 구워 두면 두 번 곱해져 시커메진다.
  const 자리 = 면.attributes.position;
  const 색 = new Float32Array(자리.count * 3);
  const 위 = 구렁이모형.폭.y || 1;
  for (let i = 0; i < 자리.count; i++) {
    // 0 = 땅에 닿은 배, 1 = 볕 받는 등
    const t = THREE.MathUtils.clamp(자리.getY(i) / 위, 0, 1);
    const v = 1.22 - 0.52 * Math.pow(t, 0.8);
    색[i * 3] = v;
    색[i * 3 + 1] = v;
    색[i * 3 + 2] = v;
  }
  면.setAttribute("color", new THREE.BufferAttribute(색, 3));
  // 표본은 여러 벌을 돌려주는 규약이다(무리가 골라 쓴다). 모형은 한 벌뿐이라
  // 그 한 벌만 담는다 — 배열로 돌려주는 모양은 지킨다.
  return [면];
}
