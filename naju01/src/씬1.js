...
export function 씬1자리들({ 지면높이 }) {
  ...
  return {
    ...
    //    (23.0, 40.0) 손으로 깎은 긴 몸일 때는 보였는데, Meshy 모형으로
    //                 바꾸니 또아리라 낮고 작아져 **0.0 %** 로 사라졌다.
    //  → 지금 자리 (19.5, 39.0) 은 배에서 내리는 자리(13, 42)에서
    //    **또아리 둘레 100 % 가 가림 없이** 드러나고, 딛는 자리는 Z1 100 %,
    //    통발·그물틀 무리와도 안 겹치는 열린 모래밭이다.
    //    내린 자리 화면 점유 **1.1 %** — 손으로 깎았을 때(0.4 %)보다 낫다.
    //
    //  [회전을 왜 저 값으로 두나]  모형의 **쳐든 머리 끝**이 로컬에서
    //    +Z 로부터 34.2° 쪽에 있다(`도구/모형굽기.mjs` 로 재 본 값).
    //    사람 쪽 방위 −65.2° 에서 그 34.2° 를 빼면 −1.735 rad 이다.
    //    ③ 이 말하는 「멀리서 서서히 다가오는」 그림이, 움직이지 않는
    //    그레이박스에서도 읽히도록 **머리를 사람 쪽으로** 돌려 둔 것이다.
    //  ※ T1 은 §5 Scene 01 에서 추락 지점으로 가는 길이라 막히면 안 된다.
    구렁이: 두기([{ x: 19.5, z: 39.0, 키: 2.6, 회전: -1.735, 색: 씬1결.구렁이등 }]),
  };
}
