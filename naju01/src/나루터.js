...
//
// [원점은 **밑동**이다 — 한 번 내렸다가 되돌렸다]
//   구운 모형은 밑동이 y=0 이다. 처음엔 그대로 넣었더니 「물 위에 얹힌 것
//   같다」 싶어(높이 1.31 m 중 1.18 m 가 수면 위) 표본을 0.3 만큼 내려
//   잠기게 했다. 그랬더니 이번엔 **바닥에 파묻혀** 보였다(사용자 지적).
//   → 되돌렸다. 물에 잠긴 정도는 **자리의 `y`** 로 정하는 게 맞다. 그건
//     배마다 다르고(대 놓은 배 · 뭍에 올린 배), 편집기로 눈으로 잡을 수 있다.
//     표본이 미리 잠겨 있으면 그 조절 폭을 표본이 빼앗는다.
const 잠기는비율 = 0;
// 색만 따로 — 배 밑동 이만큼은 젖은 빛으로 칠한다. **모양은 안 건드린다.**
//   기하와 색을 한 값으로 묶어 뒀더니, 높이를 되돌리면서 젖은 빛까지
//   같이 사라졌다. 하는 일이 다르면 값도 나눠야 한다.
const 젖는비율 = 0.22;
