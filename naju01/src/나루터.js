...
// ── 팔레트에 올릴 나루터 표본 ────────────────────────────────
// [왜 따로 만드나]
//   `나루만들기` 는 **월드 좌표로 바로 굽는다**(물가·바닥높이를 받아 그 자리에
//   말뚝을 박는다). 손으로 아무 데나 놓으려면 「길이 1 · 밑동 원점」인 표본이
//   있어야 한다 — 자리·크기·방위는 인스턴스가 갖는다(배치.js 머리말 참고).
//
// [규약]
//   · **국소 +Z 로 뻗는다** — 물가에 서서 물 쪽을 보는 방향이다.
//     반대편 물가에 놓을 때는 `회전: Math.PI` 로 돌린다.
//   · 원점은 **뭍 쪽 끝, 지면 높이**다. 그래야 물가 선에 딱 붙여 놓을 수 있다.
//   · 길이 1 로 **균등 축소**한다. 폭·높이가 길이에 비례해 따라오므로
//     `키`(= 나루 길이 m) 하나만 주면 비례가 유지된다.
export function 나루표본들(수 = 3, 시드 = 8801) {
  const 표본 = [];
  for (let i = 0; i < 수; i++) {
    const 폭 = 4.6 + (i % 3) * 0.7; // m — 널 너비를 조금씩 달리한다
    const g = 나루만들기({
      X: [-폭 / 2, 폭 / 2],
      물가: 0,
      뻗음: 3.2 + (i % 2) * 0.8,
    ...
    const 길이 = Math.max(1e-6, bb.max.z - bb.min.z);
    // 뭍 쪽 끝을 원점으로, x 는 한복판으로. y 는 `바닥높이: 0` 이라 이미 지면이 0 이다.
    g.translate(-(bb.min.x + bb.max.x) / 2, 0, -bb.min.z);
